...
from constants import DB_AUTH_NAME, FLD_BASIC_SCHEMAS
...
async def create_basic_collections(db: AsyncIOMotorClient,
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.json'):
            ...
            try:
                await db[DB_AUTH_NAME].create_collection(collection_name, validator={'$jsonSchema': schema})
                logger.info(f'Created collection {collection_name} in DB: {DB_AUTH_NAME}')
            ...
            for index in indexes:
                ...
                try:
                    await db[DB_AUTH_NAME][collection_name].create_index(list(keys.items()), **options)
